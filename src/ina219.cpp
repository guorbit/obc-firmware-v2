 ...
 *
 * 6 May 2012 by John De Cristofaro, updated November 2025 by Islay Ross for GU
 * Orbit use.
 *
...
void INA219::begin(TwoWire *theWire, uint8_t addr) {
  _i2c = theWire;
  i2c_address = addr;
  // Wire.begin();  // start i2c - now done in initEPS
}
...
void INA219::calibrate(float shunt_val, float v_bus_max, float i_max_expected) {
  ...
  current_lsb =
      i_max_expected / 32767.0f; // minimum LSB based on max expected current
...
// config values (range, gain, bus adc, shunt adc, mode) can be derived from
// pp26-27 in the datasheet defaults are: range = 1 (0-32V bus voltage range)
...
void INA219::configure(uint16_t config) { write16(INA219_REG_CONFIG, config); }
...
int16_t INA219::shuntVoltageRaw() { return read16(INA219_REG_V_SHUNT); }
...
int16_t INA219::busVoltageRaw() { return read16(INA219_REG_VBUS); }
...
int16_t INA219::read16(uint8_t reg) {
  ...
  // Exit if I2C communication failed
  if (result != 0) {
    return -1; // Return 0 on I2C communication error
  }

    // STEP 2: Read 2 bytes from that register

    uint8_t bytesRecieved = _i2c->requestFrom(i2c_address, (uint8_t)2);
    if (bytesRecieved == 2) {
      uint16_t ret = _i2c->read();
      ret <<= 8;
      ret |= _i2c->read();
      return (int16_t)ret;
    }
